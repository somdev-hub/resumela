...
const paginateTwoColumns = (sections, availableHeight) => {
  ...
  const firstPageAvailable = availableHeight - (hasHeader ? initialReserved : 0);
  ...
  while (sectionIndex < sections.length) {
    ...
    const sectionHeight = estimateSectionHeight(section);
    const pageAvailableHeight = isFirstPage ? firstPageAvailable + (hasHeader ? initialReserved : 0) : subsequentPageAvailable;
  ...
  if (currentPage.leftSections.length > 0 || currentPage.rightSections.length > 0) {
    newPages.push(currentPage);
  }
